...
def matern32_to_lti(hyperparameters: npt.NDArray, dt: float) -> tuple[npt.NDArray, npt.NDArray, npt.NDArray, npt.NDArray, npt.NDArray]:
    ''' Define SDE matrices using Matern 3/2 kernel
        :param hyperparameters: Kernel hyperparameters [l, σ]
    ...
    A = np.array(anp.exp(-dt * lam) * (dt * A_tmp + np.identity(2)))
...
class System:
    ...
    def matern32_to_lti(self, hyperparameters: npt.NDArray):
        dt = self.times[1] - self.times[0]
        return matern32_to_lti(hyperparameters, dt)